...
nets = dict(
    ...
    gabencoin=math.Object(
        ...
        ADDRESS_VERSION=16,
        RPC_PORT=56883,
        RPC_CHECK=defer.inlineCallbacks(lambda gabencoind: defer.returnValue(
            'gabencoinaddress' in (yield gabencoind.rpc_help()) and
            not (yield gabencoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 1000*100000000 >> (height + 1)//840000,
        POW_FUNC=lambda data: pack.IntType(256).unpack(__import__('ltc_scrypt').getPoWHash(data)),
        BLOCK_PERIOD=60, # s
        SYMBOL='GBN',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'Gabencoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/Gabencoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.gabencoin'), 'gabencoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='https://blockchain.info/block/',
        ADDRESS_EXPLORER_URL_PREFIX='https://blockchain.info/address/',
        TX_EXPLORER_URL_PREFIX='https://blockchain.info/tx/',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256//1000 - 1),
    ...
    gabencoin_testnet=math.Object(
        ...
        RPC_CHECK=defer.inlineCallbacks(lambda gabencoind: defer.returnValue(
            'gabencoinaddress' in (yield gabencoind.rpc_help()) and
            (yield gabencoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 1000*100000000 >> (height + 1)//840000,
        POW_FUNC=lambda data: pack.IntType(256).unpack(__import__('ltc_scrypt').getPoWHash(data)),
        BLOCK_PERIOD=60, # s
    ...
    penguincoin=math.Object(
        P2P_PREFIX='fdcab4dc'.decode('hex'),
        P2P_PORT=39533,
        ADDRESS_VERSION=117,
        RPC_PORT=39532,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'PenguinCoinaddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 500000*100000000 >> (height + 1)//100000,
        POW_FUNC=lambda data: pack.IntType(256).unpack(__import__('ltc_scrypt').getPoWHash(data)),
        BLOCK_PERIOD=150,
        SYMBOL='PENG',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'Penguincoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/Penguincoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.penguincoin'), 'penguincoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://coincrawler.de:55/block/',
        ADDRESS_EXPLORER_URL_PREFIX='http://coincrawler.de:55/address/',
        TX_EXPLORER_URL_PREFIX='http://coincrawler.de:55/tx/',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256//1000 - 1),
        DUMB_SCRYPT_DIFF=2**16,
        DUST_THRESHOLD=0.03e8,
    ),
    penguincoin_testnet=math.Object(
        P2P_PREFIX='fcc1b7dc'.decode('hex'),
        P2P_PORT=29533,
        ADDRESS_VERSION=113,
        RPC_PORT=29532,
        ...
        SYMBOL='tPENG',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'Penguincoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/Penguincoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.penguincoin'), 'penguincoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://nonexistent-penguincoin-testnet-explorer/block/',
        ADDRESS_EXPLORER_URL_PREFIX='http://nonexistent-penguincoin-testnet-explorer/address/',
        TX_EXPLORER_URL_PREFIX='http://nonexistent-penguincoin-testnet-explorer/tx/',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256 - 1),
        DUMB_SCRYPT_DIFF=2**16,
        DUST_THRESHOLD=1e8,
    ),
